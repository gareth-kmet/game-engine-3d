...
import rendering.loaders.collada.structures.JointTransformData;
import rendering.loaders.collada.structures.KeyFrameData;
...
public class AnimationLoader {
	...
	public AnimationData extractAnimation(){
		animName = animationData.getChild("animation").getAttribute("name")+'_';
		animNameLength = animName.length();
	...
	private float[] getKeyTimes(){
		XmlNode animationNode = animationData.getChild("animation");
		XmlNode sourceNode = animationNode.getChild("source");
		if(sourceNode==null) {
			animationNode=animationNode.getChild("animation");
			sourceNode = animationNode.getChild("source");
		}
		XmlNode timeData = sourceNode.getChild("float_array");
	...
	private String getJointName(XmlNode jointData){
		...
		if(name.length()>animNameLength&&name.substring(0,animNameLength).equals(animName)) {
			name=name.substring(animNameLength);
		}
		return name;
	}
...
//				Matrix4f.mul(CORRECTION, transform, transform);
	...
	private String findRootJointName(){
		XmlNode skeleton = jointHierarchy.getChild("visual_scene").getChildWithAttribute("node", "type", "NODE");
