...
public class ShadowMapMasterRenderer {
	 ...
	 */
	public ShadowMapMasterRenderer(Loader loader) {
		shader = new ShadowShader();
		shadowBox = new ShadowBox(lightViewMatrix);
		shadowFbo = new ShadowFrameBuffer(SHADOW_MAP_SIZE, SHADOW_MAP_SIZE);
		entityRenderer = new ShadowMapEntityRenderer(loader, shader, projectionViewMatrix);
	}
	 ...
	 */
	public void render(GivenMainLoopRequiredVariables variables) {
		shadowBox.update(variables.camera());
		Vector3f sunPosition = variables.sun().getPosition();
		Vector3f lightDirection = new Vector3f(-sunPosition.x, -sunPosition.y, -sunPosition.z);
		prepare(lightDirection, shadowBox);
		entityRenderer.render(variables.entities());
		finish();
	}
